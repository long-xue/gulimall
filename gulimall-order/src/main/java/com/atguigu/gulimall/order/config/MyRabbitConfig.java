...
//	@Autowired
	...
	/**
	 * 1.设置确认回调： ConfirmCallback  publisher->broke
	 * 先在配置文件中开启 publisher-confirms: true
	 * @PostConstruct: MyRabbitConfig对象创建完成以后 执行这个方法
	 *
	 *  2.消息抵达队列的确认回调
	 * 　	开启发送端消息抵达队列确认 ReturnCallback broke->queue
	 *     publisher-returns: true
	 *     	只要抵达队列，以异步优先回调我们这个 returnconfirm(比)
	 *     template:
	 *       mandatory: true
	 *	3.消费端确认(保证每一个消息被正确消费broker
	 *才可以删除消息)
	 *		1.默认是自动确认的 只要消息接收到 服务端就会移除这个消息
	 *
	 *		如何签收:
	 *			签收: channel.basicAck(deliveryTag, false);
	 *			multiple:表示批量，设置为true表示该通道内该消息以前的消息都批量通过
	 *			拒签: channel.basicNack(deliveryTag, false,true);
	 *			deliveryTag通道内自增的long数据
	 *	配置文件中一定要加上这个配置
	 *		listener:
	 *       simple:
...
//	@PostConstruct
	public void initRabbitTemplate(){
		 ...
		 *  ack（acknowledgment）消息回执： 消息是否成功收到
		 * 	cause：失败的原因
		 */
